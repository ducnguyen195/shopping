                    ...
                    <ol class="inline-flex items-center space-x-1 md:space-x-2">
                        ...
                        <li>
                            <div class="flex items-center">
                                ...
                                <a href="#" class="ml-1 text-sm font-medium text-gray-700 hover:text-gray-900 md:ml-2">Ảnh</a>
                            </div>
                        </li>
                        <li>
                            <div class="flex items-center">
                                ...
                                <span class="ml-1 text-sm font-medium text-gray-400 md:ml-2" aria-current="page"> </span>
                            </div>
                        </li>
                    </ol>
                </nav>
                <h1 class="text-xl font-semibold text-gray-900 sm:text-2xl">Tất cả ảnh {{$type_name}}</h1>
                <div class="block items-center sm:flex md:divide-x md:divide-gray-100">
                    <div class="flex items-center w-full sm:justify-end">
                        <a href="{{route('admin.add_image',$type)}}">
                ...
                <div class="overflow-hidden">
                    <table class="w-full text-sm text-left text-gray-500 dark:text-gray-400">
                        ...
                        <tr class="hover:bg-gray-100">
                            ...
                            <td class="p-4 text-base font-medium text-gray-900  lg:p-5  ">
                               <p class="w-60 flex gap-4">
                                   @php
                                       $image_paths = json_decode($item->path);
                                   @endphp
                                   @if(count($image_paths) == 1)
                                       <img src="{{ asset($image_paths[0])}}" alt="{{$item->alt}}" class="w-16 h-12 flex gap-2">
                                   @else
                                       @foreach(array_slice($image_paths,0,2) as $path)
                                           <img src="{{ asset($path) }}" alt="{{$item->alt}}" class="w-16 h-12 flex gap-2">
                                       @endforeach
                                   @endif

                               </p>
                            </td>
                            <td class="p-4 text-base font-medium text-gray-900 whitespace-nowrap lg:p-5">{{$item ->alt}}</td>
                            <td class="p-4 text-base font-normal text-gray-900 whitespace-nowrap lg:p-5"><p>{{ $item ->products->name}}</p></td>
                            <td class="p-4 space-x-2 whitespace-nowrap lg:p-5">
                                <a href="{{route('admin.edit_image',[$type,$item->id])}}">
                                    <button type="button" data-modal-target="edit-brand-modal" data-modal-toggle="edit-brand-modal"
                                ...
                                <a href="#">
                                    <button type="button" data-modal-target="delete-image-modal-{{$item->id}}" data-modal-toggle="delete-image-modal-{{$item->id}}" class="inline-flex items-center py-2 px-3 text-sm font-medium text-center text-white bg-gradient-to-br from-red-500 to-red-600 rounded-lg shadow-md shadow-gray-300 hover:scale-[1.02] transition-transform">
                    ...
                    {{--   Modal delete--}}
                        <div class="hidden overflow-y-auto overflow-x-hidden fixed right-0 left-0 top-4 z-50 justify-center items-center md:inset-0 h-modal sm:h-full" id="delete-image-modal-{{$item->id}}" aria-hidden="true">
                            <div class="relative px-4 w-full max-w-md h-full md:h-auto">
                                <div class="relative bg-white rounded-2xl shadow-lg">
                                    <div class="flex justify-end p-2">
                                        <button type="button" class="text-gray-400 bg-transparent hover:bg-gray-200 hover:text-gray-900 rounded-2xl text-sm p-1.5 ml-auto inline-flex items-center" data-modal-toggle="delete-image-modal">
                                    ...
                                    <div class="p-6 pt-0 text-center">
                                        ...
                                        <a href="#" class="text-white bg-gradient-to-br from-red-400 to-red-600 font-medium rounded-lg text-base inline-flex items-center px-3 py-2.5 text-center mr-2 shadow-md shadow-gray-300 hover:scale-[1.02] transition-transform">
                                            Yes, I'm sure
                                        </a>
