...
@section('content')
    <div class="">
        ...
        <div class="p-4 mx-4 mt-4 mb-4 bg-white rounded-2xl shadow-xl shadow-gray-200 lg:p-5  ">
         <form action="{{route('admin.update_post',$post->id)}}" method="POST" class="w-full h-full" >
             @csrf
             ...
             <div class="grid grid-cols-2 p-3 gap-x-6 border mt-5 rounded-2xl">
                 <div class="col-span-1">
                     <div class="mt-5">
                         ...
                         <input type="text" id="post_name" name="name" class="rounded-lg w-full mt-2" value="{{$post->name}}">
                     </div>
                     <div class="mt-5">
                         <label for="post_slug"> Slug</label>
                         <input type="text" id="post_slug" name="slug" class="rounded-lg w-full mt-2" value="{{$post->slug}}">
                     </div>
                     <div class="mt-5">
                         <label for="post_description" class="block mb-2" > Description</label>
                         <textarea type="text" id="post_description" name="description" class="rounded-lg w-full" > {!! $post->description !!}</textarea>
                     </div>
                 </div>
                 <div class="col-span-1">
                     <div class="mt-5">
                         <label for="post_seo_title" class="block"> SEO Title</label>
                         <input type="text" id="post_seo_title" name="seo_title" class="rounded-lg w-full mt-2" value="{{$post->seo_title}}">
                     </div>
                     <div class="mt-5">
                         <label for="post_seo_keyword" class="block"> SEO Keyword</label>
                         <input type="text" id="post_seo_keyword" name="seo_keywords" class="rounded-lg w-full mt-2" value="{{$post->seo_keywords}}">
                     </div>
                     <div class="mt-5">
                         <label for="post_seo_description" class="block mb-2"> SEO Description</label>
                         <textarea type="text" id="post_seo_description" name="seo_description" class="rounded-lg w-full " >{!! $post->seo_description !!} </textarea>
                     </div>
                 </div>
             </div>
             <div class="mt-5 ">
                 <div class="p-3 border rounded-2xl ">
                     ...
                     <textarea name="content" id="post_content" class="w-full rounded-lg"> {!! $post->content !!} </textarea>
                 </div>
             </div>
