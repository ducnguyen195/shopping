...
@section('content')
    <div>
        <div class="m-5 p-5 rounded-2xl shadow-lg shadow-gray-200 bg-white">
            ...
            <div>
                <h2 class="font-bold text-2xl"> Edit Brand</h2>
            </div>
            <div>
        ...
        <div class="m-5 p-5 grid grid-cols-2  " >
            <form action="{{route('admin.update_brand',$brand->id)}}"  method="POST" class="w-full col-span-1 self-center rounded-2xl shadow-lg shadow-gray-200 bg-white " >
                @csrf
                <div class=" mt-5 gap-4 px-10 ">
                    <div class="">
                        <div class="" >
                            <label for="brand_name" class="block mb-2 font-medium text-gray-600"> Brand Name</label>
                            <input type="text" id="brand_name" name="name" class=" rounded-lg outline-violet-300 w-full " value="{{$brand->name}}" required>
                        </div>
                        <div class=" mt-5">
                            <label for="brand_slug" class=" block mb-2 font-medium text-gray-600" > Slug </label>
                            <input type="text" id="brand_slug" class=" rounded-lg outline-violet-300 w-full  " value="{{$brand->slug}}" >
                        </div>
                    </div>
                    <div class="mt-5">
                        <label for="add_description" class="block mb-2 font-medium text-gray-600 "> Description</label>
                        <textarea type="text" id="add_description" name="description"  class=" rounded-lg  w-full" > {{$brand->description}} </textarea>
                    </div>
