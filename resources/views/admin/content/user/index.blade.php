                        ...
                        <table class="min-w-full divide-y divide-gray-200 table-fixed">
                            ...
                            @foreach($shops as $item)
                                <tr class="hover:bg-gray-100">
                                    ...
                                    <td class="p-4 text-base font-medium text-gray-900 whitespace-nowrap lg:p-5">{{$item -> name}}</td>
                                    <td class="p-4 text-base font-medium text-gray-900 whitespace-nowrap lg:p-5">{{$item -> address}}</td>
                                    <td class="p-4 text-base font-medium text-gray-900 whitespace-nowrap lg:p-5">{{$item -> phone}}</td>
                                    <td class="p-4 text-base font-normal text-gray-900 whitespace-nowrap lg:p-5">
                                        @php
                                            $image_path = json_decode( $item->image_path)
                                        @endphp
                                        @if($image_path)
                                            @foreach($image_path as $image)
                                                <img src="{{asset($image)}}" alt="" class="w-10 h-14">
                                            @endforeach
                                        @endif
                                    </td>
                                    <td class="p-4 text-base font-normal text-gray-900 whitespace-nowrap lg:p-5">{{ $item ->google_map }}</td>
